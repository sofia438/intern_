...
export default async function Page({ searchParams }: { searchParams: Promise<{ plan?: string }> }) {
  ...
  return (
    <main className="p-8">
      <div className="mb-10">
        <h1 className="text-5xl font-black tracking-tight">{t.billingCheckoutPage.title}</h1>
        <p className="mt-2 text-xl text-neutral-600 dark:text-neutral-400">
          {t.billingCheckoutPage.subtitle}
        </p>
      </div>

      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.4fr]">
        <div className="h-fit rounded-md border border-[#dfe2e7] bg-white p-7 dark:border-[#3a3a3a] dark:bg-[#242424]">
          <h2 className="font-mono text-sm uppercase tracking-[0.12em] text-neutral-500 dark:text-neutral-400">{plan.name} {t.billingCheckoutPage.planSuffix}</h2>
          <div className="mt-2">
            <span className="text-4xl font-black dark:text-white">${plan.price}</span>
            <span className="text-neutral-500 dark:text-neutral-400"> {t.billingCheckoutPage.perMonth}</span>
          </div>
          ...
          <div className="mt-6 space-y-2 border-t border-[#ececec] pt-6 text-sm dark:border-[#3a3a3a] dark:text-neutral-200">
            <div className="flex justify-between">
              <span className="text-neutral-600 dark:text-neutral-400">{t.billingCheckoutPage.subtotal}</span>
              <span>${plan.price.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-neutral-600 dark:text-neutral-400">{t.billingCheckoutPage.taxVat}</span>
              <span className="text-neutral-500 dark:text-neutral-400">{t.billingCheckoutPage.taxCalculated}</span>
            </div>
            <div className="flex justify-between border-t border-[#ececec] pt-2 text-lg font-black dark:border-[#3a3a3a] dark:text-white">
              <span>{t.billingCheckoutPage.totalToday}</span>
              <span>${plan.price.toFixed(2)}</span>
            </div>
          </div>
        </div>
